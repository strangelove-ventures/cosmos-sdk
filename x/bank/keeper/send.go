...
var blockedAddresses map[string]bool
...
func (k BaseSendKeeper) BlockedAddr(addr sdk.AccAddress) bool {
	return blockedAddresses[addr.String()]
}
...
func (k BaseSendKeeper) UpdateBlockedAddresses(add []string, remove []string) {
	...
	for _, addr := range remove {
		...
		delete(blockedAddresses, ondoAddr.String())
		delete(blockedAddresses, hexAddr.Hex())
	}
	...
	for _, addr := range add {
		...
		blockedAddresses[ondoAddr.String()] = true
		blockedAddresses[hexAddr.Hex()] = true
	}
}
